...
public class BestCurveFittingCompression implements LossyCompression {
    ...
    public static void main(String[] args) throws IOException {
        ...
        String inputFilePath = "E:\\实验室学习\\项目\\数据压缩\\UCELL.csv\\clustering\\UCELLnew1(c).csv";
        String outputFilePath = "E:\\实验室学习\\项目\\数据压缩\\UCELL.csv\\curveOutpout\\output0320.csv";
...
//            ArrayList<String> tempList = new ArrayList<>();
            ...
            if (objectId.equals(""))
                ...
                cb = compressionBaseObject(allList);//基于列的压缩
//                cb = compressionObjectBaseRow(allList);//基于行的压缩
...
//        cb = compressionObjectBaseRow(allList);//基于行的压缩
    ...
    public void regressionBasedcurveFitingCompression(String inputFilePath,String outputFilePath) throws  IOException{
        ...
        ArrayList<ArrayList<String>>  groupIndexArr=readIndexLine(indexLine);
        ...
        //第三步读取各分组的数据
        ArrayList<ArrayList<String>> allGroupData=new ArrayList<>();
        ArrayList<String>  oneGroupData=new ArrayList<>();
        while((line=reader.readLine())!=null){
            if(!line.equals("#"))
                 oneGroupData.add(line);
            else {
                allGroupData.add(oneGroupData);
     ...
     */
    public void groupBasedCompression(ArrayList<String> oneGroupData,ArrayList<String> ws){
     ...
     */
    public Bit oneRowOfGroupBasedCompression(String rowOfGroup,ArrayList<String> ws){
        Bit res = new Bit();
        ArrayList<String> columnList= pythonListToll.getList(rowOfGroup);
...
//        Arrays.sort(allLineList,new ColumnComparator());
        ...
        for (AObject ao : objectList) {
            for (int i = 0; i < columnCount; i++) {
                allLineList[i].addVal(ao.getVal(i));
                if (isZero.get(i))
                    isZero.set(i, Float.parseFloat(ao.getVal(i)) == 0f ? true : false);
            }
        }
        for (int i = 0; i < columnCount; i++)
        ...
        for (int i = 0; i < columnCount; i++) {
            cb.getBitList().add(curveFittingCompressionBaseLine(allLineList[i].getValList(), isZero.get(i)));
        }
        return cb;
    }
     ...
     * @修改人和其它信息
     */
    public CurveObject compressionObjectBaseRow(ArrayList<AObject> objectList) {
        CurveObject cb = new CurveObject();
        cb.setStartTime(objectList.get(0).getStartTime());
        cb.setEndTime(objectList.get(objectList.size() - 1).getEndTime());
        cb.setObjId(objectList.get(0).getObjId());
        for (AObject ao : objectList) {
            ...
            for (int i = 0; i < columnCount; i++) {
                ...
                currentRowList.add(str);
            }
            cb.getBitList().add(curveFittingCompressionBaseLine(currentRowList, false));
        }
        return cb;
    }
    ...
    public void printObject(String outputFilePath, CurveObject cb) throws IOException {
        ...
        br.write(bytes);
        //第2步,输出开始时间和对象id.
        bytes = (cb.getStartTime() + cb.getObjId()).getBytes();
        br.write(bytes);
        //输出每一列的类型
